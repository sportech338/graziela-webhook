...
def objecao_foi_contornada(ultima_objecao: str, contexto: str) -> bool:
    if not ultima_objecao:
        return True
    ...
    return ultima_objecao.lower() not in texto
...
def controlar_jornada(mensagem: str, contexto: str, estado_anterior: dict = None) -> dict:
    ...
    nova_etapa, justificativa_etapa = identificar_etapa_jornada(texto_total)
    nova_objecao, justificativa_objecao = identificar_objecao(texto_total)
    nova_consciencia, justificativa_consciencia = classificar_consciencia(texto_total)
    nova_temperatura, justificativa_temperatura = classificar_temperatura(mensagem)
    ...
    if estado_anterior:
        ...
        if objecao_anterior and objecao_foi_contornada(objecao_anterior, texto_total):
            objecao = None
            justificativa_objecao = None
        else:
